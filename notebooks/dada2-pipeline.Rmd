...
Download the fastq archives via scp:

`$ scp <username>@<host_address>:</path/to/fastq/archives> /home/rstudio/dada2_pipeline/data/`

Split the fastq files into smaller files
```{bash fastq_split, eval=F}
# BASH
...
for dir in "${rundirs[@]}"; do
  ls "${dir}"*.fastq* \
    | parallel -j 1 'gunzip {}; f=$(echo {} | sed "s/\.fastq\.gz$//") \
      && split -l 100000 -d --additional-suffix=.fastq "${f}".fastq "${f}" \
      && rm {} \
      && gzip "${f}"*.fastq'
done
```

Store the archives on LFS:
```{bash lfs, eval=F}
# BASH
...
git add -A
git commit -m "moving .fastq and .fastq.gz to LFS"
```
...
```{bash extract_barcodes, eval=F}
# BASH

# For each run extract barcodes and samples names for the iu-demultiplex command
# Also create a new directory for demultiplexed sequences
ls -d ../data/*/ \
  | parallel 'cut -f1,2 {}map.txt | sed 1d > {}barcode_to_sample.txt; \
...
for dir in "${rundirs[@]}"; do
  ls "${dir}"R1*.fastq.gz \
    | parallel -j 1 'id=$(echo {/.} | sed "s/1|\.fastq//"); \
      r2={//}/$(echo {/.} | sed "s/R1/R2/"); \
      i={//}/$(echo {/.} | sed "s/R1/Index/"); \
      git lfs pull {} \
      && git lfs pull "${r2}".gz \
      && git lfs pull "${i}".gz \
      && gunzip {} \
      && gunzip "${r1}".gz \
      && gunzip "${index}".gz \
      && mkdir {//}/demultiplexed/"${id}" \
      && iu-demultiplex -s {//}/barcode_to_sample.txt --r1 {.} --r2 "${r2}" -i "${index}" -x -o {//}/demultiplexed/"${id}" \
      && gzip {} \
      && gzip "${r1}".gz \
      && gzip "${index}".gz \
      && git add .gitattributes \
      && git add -A \
      && git commit -m "moving demultiplexed .fastq to LFS"'
done
```
